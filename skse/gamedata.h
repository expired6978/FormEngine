...
#define OOLUA_FUNCMACRO(objectName, alias, member) \
	UInt32 GetNum##alias##s() \
	{ \
		return (##member##.count); \
	} \
	##objectName## * GetNth##alias##(UInt32 n) \
	{ \
		##objectName## * item = NULL; \
		##member##.GetNthItem(n, item); \
		return static_cast<##objectName##*>(item); \
	} \
...
public:
	...
	{
		return (DataHandler *)DataHandler::GetSingleton();
	}

	std::string GetModName(UInt32 modIndex)
	{
		ModInfo* modInfo = modList.modInfoList.GetNthItem(modIndex);
		return std::string(modInfo->name);
	}

	TESForm * GetFormFromFile(UInt32 lowerFormId, std::string fileName)
	{
		UInt8 modIndex = this->GetModIndex(fileName.c_str());
		if(modIndex != -1) {
			UInt32 formId = ((UInt32)modIndex) << 24 | lowerFormId;
	...
	OOLUA_FUNCMACRO(EnchantmentItem, Enchantment, enchantments)
	OOLUA_FUNCMACRO(BGSKeyword, Keyword, keywords)
	OOLUA_FUNCMACRO(BGSListForm, FormList, listForms)
};
